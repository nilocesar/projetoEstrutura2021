define(['jquery'], function($) {
    ...
    var scorm = function() {
        ...
        var $private = {};
        var $parent = {};
        var $bodyIframe = window.parent.iframe.body;
        ...
        $public.setInteractions = function setInteractions(indice, correto, resposta, tipo, tempoGasto, pesoDado) {
            ...
            $bodyIframe.scorm_set_interactions(indice, correto, resposta, tipo, tempoGasto, pesoDado); //_indice, correto, resposta, _tipo, _tempoGasto, _pesoDado
        }

        $public.setStatusLesson = function setStatusLesson(_status) {
            $bodyIframe.scorm_set_status_lesson(_status)
        }

        $public.getStatusLesson = function getStatusLesson() {
            return $bodyIframe.scorm_get_status_lesson()
        }

        $public.getInteractionsCount = function getInteractionsCount() {
            return $bodyIframe.scorm_get_interactions_count()
        }

        $public.resetAll = function resetAll() {
            $bodyIframe.scorm_reset();
        }

        $public.resetSuspendata = function resetSuspendata() {
            $bodyIframe.scorm_reset_suspendData();
        }

        $public.resetLessonLocation = function resetLessonLocation() {
            $bodyIframe.scorm_reset_lessonLocation();
        }

        $public.sairScorm = function sairScorm() {
            $bodyIframe.scorm_quit();
        }
